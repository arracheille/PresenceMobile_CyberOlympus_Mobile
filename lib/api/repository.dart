import 'dart:convert';
import 'dart:io';
import 'package:project_test/model/workspace.dart';
...
class Repository {
  ...
  Future<Map<String, dynamic>> fetchPosts(int page) async {
    final response = await http.get(Uri.parse('$baseUrl/workspaces?page=$page'));

    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      List<Workspace> workspaces = (data['data']['data'] as List)
          .map((postJson) => Workspace.fromJson(postJson))
          .toList();
      return {
